...
private:
    ...
    void collect(std::shared_ptr<Node> node, std::vector<char>& path, 
                std::vector<std::vector<char>>& perms) const;
    bool findTraversal(std::shared_ptr<Node> node, std::vector<char>& path, 
                      int& count, int target_num, std::vector<char>& res) const;
    bool findNavigation(std::shared_ptr<Node> node, std::vector<char>& path, 
